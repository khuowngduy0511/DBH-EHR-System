...
public class EhrService : IEhrService
{
    ...
    public async Task<PaginatedResult<EhrRecordResponseDto>> GetMyVisibleRecordsAsync(int page, int pageSize, string? search)
    {
        ...
        var (items, totalCount) = await _ehrRecordRepo.GetAccessibleRecordsPaginatedAsync(orgId, consentedEhrIds, consentedPatientIds, search, matchingUserIds, page, pageSize);
