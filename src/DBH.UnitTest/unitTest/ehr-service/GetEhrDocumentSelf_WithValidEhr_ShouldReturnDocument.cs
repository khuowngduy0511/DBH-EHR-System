...
/// <summary>
/// Good case: Patient retrieves their own EHR document without X-Requester-Id header
/// Expected: 200 OK with document content
/// </summary>
public class EhrServiceTests_GetEhrDocumentSelf_WithValidEhr_ShouldReturnDocument : ApiTestBase
{
    ...
    [SkippableFact]
    public async Task GetEhrDocumentSelf_WithValidEhr_ShouldReturnDocument()
    {
        // Authenticate as doctor who can create records
        ...
        // Step 2: Get document self (current user access)
        var getResponse = await GetWithRetryAsync(EhrClient, ApiEndpoints.Ehr.GetDocumentSelf(ehrId));

        // Should succeed or return 403/404 depending on permissions
        Assert.True(
            ...
            $"Expected 200, 403, or 404, got {getResponse.StatusCode}");
    }
}
