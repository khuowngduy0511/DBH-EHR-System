...
/// <summary>
/// Good case: Retrieve EHR document with valid requester ID
/// Expected: 200 OK with document content
/// </summary>
public class EhrServiceTests_GetEhrDocument_WithValidRequesterId_ShouldReturnDocument : ApiTestBase
{
    ...
    [SkippableFact]
    public async Task GetEhrDocument_WithValidRequesterId_ShouldReturnDocument()
    {
        ...
        // Step 2: Get document with valid requester ID (doctor can access their own records)
        var getRequest = EhrClient.CreateRequest(HttpMethod.Get, ApiEndpoints.Ehr.GetDocument(ehrId));
        getRequest.Headers.Add("X-Requester-Id", TestSeedData.DoctorUserId.ToString());

        var response = await EhrClient.SendAsync(getRequest);

        // Document retrieval may return 200 OK or 403 Forbidden depending on consent
        Assert.True(
            response.StatusCode == HttpStatusCode.OK || 
            response.StatusCode == HttpStatusCode.Forbidden ||
            response.StatusCode == HttpStatusCode.NotFound,
            $"Expected 200, 403, or 404, got {response.StatusCode}");
    }
}
