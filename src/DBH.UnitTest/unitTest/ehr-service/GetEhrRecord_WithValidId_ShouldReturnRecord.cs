...
/// <summary>
/// Good case: Retrieve an existing EHR record
/// Expected: 200 OK with EHR record data
/// </summary>
public class EhrServiceTests_GetEhrRecord_WithValidId_ShouldReturnRecord : ApiTestBase
{
    ...
    [SkippableFact]
    public async Task GetEhrRecord_WithValidId_ShouldReturnRecord()
    {
        ...
        // Step 2: Retrieve the created record
        var getResponse = await GetWithRetryAsync(EhrClient, ApiEndpoints.Ehr.GetRecord(ehrId));

        Assert.Equal(HttpStatusCode.OK, getResponse.StatusCode);
        
        var getJson = await ReadJsonResponseAsync(getResponse);
        Assert.True(getJson.TryGetProperty("ehrId", out var retrievedEhrId));
        Assert.Equal(ehrId.ToString(), retrievedEhrId.GetString());
    }
}
